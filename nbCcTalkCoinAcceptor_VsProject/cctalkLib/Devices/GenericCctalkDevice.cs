...
namespace dk.CctalkLib.Devices
{
    ...
    {
        static ILog log = LogManager.GetLogger("AppLog");
        ...
        protected ICctalkChecksum _checksumHandler = new CRCChecksum();
        ...
        public GenericCctalkDevice()
        {

        }


        protected CctalkMessage CreateMessage(CctalkCommands header)
        {
        ...
        {
            var msg = CreateMessage(CctalkCommands.ResetDevice);
        ...
        public CctalkMessage CmdSimplePoll()
        {
            var msg = CreateMessage(CctalkCommands.SimplePoll);

            return Connection.Send(msg, _checksumHandler);
        }

        public TimeSpan CmdRequestPollingPriority()
        {
            var msg = CreateMessage(CctalkCommands.RequestPollingPriority);
        ...
        {
            var msg = CreateMessage(CctalkCommands.RequestStatus);
        ...
        {
            return RequestForStringHelper(CctalkCommands.RequestManufacturerId);

        }
        public CctalkDeviceTypes CmdRequestEquipmentCategory()
        {
            var catName = RequestForStringHelper(CctalkCommands.RequestEquipmentCategoryId);
        ...
        {
            return RequestForStringHelper(CctalkCommands.RequestProductCode);
        }

        public String CmdRequestSoftwareRevision()
        {
            return RequestForStringHelper(CctalkCommands.RequestSoftwareVersion);
        }

        public DeviceEventBuffer CmdReadEventBuffer()
        {
            var msg = CreateMessage(CctalkCommands.RequestBufferedCreditOrErrorCodes);
            var respond = Connection.Send(msg, _checksumHandler);
            //CmdReset();
        ...
        {
            var msg = CreateMessage(CctalkCommands.RequestSerialNumber);
            var respond = Connection.Send(msg, _checksumHandler);
        ...
        public void CmdSetMasterInhibitStatus(Boolean isInhibiting)
        {
            ...
            var msg = CreateMessage(CctalkCommands.ModifyMasterInhibitStatus);
            msg.Data = new[] { (Byte)(isInhibiting ? 0 : 0xFF) };
            var res = Connection.Send(msg, _checksumHandler);

            bool _isInhibiting = CmdGetMasterInhibitStatus();
            if (isInhibiting != _isInhibiting)
            {
                Console.WriteLine("error setting the MasterInhibitStatus to: " + isInhibiting);
                log.Debug("error setting the MasterInhibitStatus to: " + isInhibiting);
            }
            // CmdReset();
        ...
        {
            var msg = CreateMessage(CctalkCommands.RequestMasterInhibitStatus);
            var respond = Connection.Send(msg, _checksumHandler);
            if (respond.DataLength < 1)
                throw new InvalidRespondException(respond);
        ...
        */
        ...
        {
            var msg = CreateMessage(CctalkCommands.ModifyInhibitStatus);
            msg.Data = new[] { (Byte)Data1, (Byte)Data2 };
            Connection.Send(msg, _checksumHandler);

            //if (Data1 != 0 || Data2 != 0)
            {
                //CmdSetMasterInhibitStatus(Data1 == 0 || Data2 != 0);
            }


            byte[] ret = CmdRequestInhibitStatus();
            
            if (ret == null || ret[0] != Data1 || ret[1] != Data2)
            {
                Console.WriteLine("error setting coinvalues to be accepted");
                log.Debug("error setting coinvalues to be accepted");
            }
        ...
        {
            var msg = CreateMessage(CctalkCommands.RequestInhibitStatus);
            var ret = Connection.Send(msg, _checksumHandler);
            if (ret.DataLength != 2) { Console.WriteLine("returning null"); return null; }
            Console.WriteLine(ret.Data[0] + " " + ret.Data[1]);
            return new byte[] { ret.Data[0], ret.Data[1] };
        }




        internal bool TestSolenoids()
        {
            var msg = CreateMessage(CctalkCommands.TestSolenoids);
            var response = Connection.Send(msg, _checksumHandler);
            return response.Header == 0;
        }

        internal bool TestOutputLines()
        {
            var msg = CreateMessage(CctalkCommands.TestOutputLines);
            var response = Connection.Send(msg, _checksumHandler);
            return response.Header == 0;
        }
        #endregion

        String RequestForStringHelper(CctalkCommands header)
        {
        ...
        public static CctalkMessage ParseResponse(Byte[] source, Int32 offset, Int32 length)
        {
            var ret = ParseMessage(source, offset, length);
            if (ret.Header != 0)
            {
                Console.WriteLine("ret.Header should be zero: " + ret.Header);
                //throw new InvalidRespondFormatException(source, "Invalid response header. Possible reason: echo is enabled");
            }
            return ret;
        }
        ...
        public static CctalkMessage ParseMessage(Byte[] source, Int32 offset, Int32 length)
        {
            ...
            var ret = new CctalkMessage
                          {
                              Header = (CctalkCommands)source[CctalkMessage.PosHeader + offset],
        ...
        public static Boolean IsResponseComplete(Byte[] respondRawData, Int32 lengthOverride)
        {
        ...
        {
            return IsResponseComplete(respondRawData, respondRawData.Length);
        }
